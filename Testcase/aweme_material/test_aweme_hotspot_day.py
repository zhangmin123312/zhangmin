...
list_dates =  base.return_hotspot_time(os.getenv("token"),os.getenv("host"),data_type='day')
...
class TestCase_Aweme_Hotspot_day():
    ...
    @pytest.mark.parametrize('list_date',list_dates)
    ...
    # @pytest.mark.parametrize('publish_time',base.return_hotspot_time(os.getenv("host"),include_today=0))
    # @pytest.mark.parametrize('list_date',[base.return_hotspot_time(os.getenv("host"),data_type='day')[-1]])
    # @allure.title("每日探测榜{publish_time}发布的视频在{day_interval}排榜情况")
    # def test_hotspot_day_interval(self,get_token,get_host,list_date,publish_time):
